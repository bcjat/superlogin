...
function isSessionExpired(sessionData) {
  ...
  if (sessionData.ends > 0) return Math.min(sessionData.ends, sessionData.expires) < Date.now();
...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
    ...
    if (userDoc.providers)
      userDoc.providers.forEach(function (providerName) {
        ...
        if (providerData) {
          ...
          if (providerData.profile && providerData.profile.roles) providerData.profile.roles.forEach(_addRole);
        }
      });
    return roles;
  };
  ...
  const userModel = {
    async: true,
    whitelist: ["name", "username", "email", "password", "confirmPassword", "groupDb"],
    customValidators: {
    ...
    return BPromise.resolve()
      ...
      .then(function (results) {
        if (results.rows.length > 0) {
          ...
          if (emailUsername) {
            if (!user.email) {
              if (profile.username && email.match(profile.username)) user.email = profile.username;
            }
            if (!user.email) {
              return BPromise.reject({
  ...
  this.createSession = function (user_id, provider, req) {
    ...
    return userDB
      ...
      .then(function (finalUser) {
        user = finalUser;
        return userDB.put(finalUser).catch(function (err) {
      ...
      .then(function () {
        ...
        if (typeof user.personalDBs === "object") {
          ...
          if (config.getItem("dbServer.publicURL")) {
            ...
            publicURL =
              config.getItem("dbServer.protocol") + newSession.token + ":" + newSession.password + "@" + config.getItem("dbServer.host") + "/";
          }
  ...
  // TODO
  userDB.changes({ since: "now", live: true, include_docs: true }).on("change", function (change) {
    if (change.deleted) {
      userDB.get(change.id, { revs: true, open_revs: "all" }).then(function (infos) {
        console.log("userDB.changes: deleted", { change, infos });
        const revs = infos[0].ok._revisions;
        const lastRev = revs.start - 1 + "-" + revs.ids[1];
        return userDB.get(change.id, { rev: lastRev }).then(function (user) {
          function _deleteDBS() {
            if (user.personalDBs && typeof user.personalDBs === "object") {
              return BPromise.all(
                Object.keys(user.personalDBs).map(function (db) {
                  var dbConfig = dbAuth.getDBConfig(db);
                  // TODO: what about "if(db.type === 'private')" ??
                  if (dbConfig.deleteWithUser) return dbAuth.removeDB(db);
                  return BPromise.resolve();
                })
              );
            }
          ...
          return _deleteDBS().then(function () {
  ...
  this.cleanupSessions = function (userDoc, op, forceSave) {
    ...
    op = op || "expired";
    return self.logoutUserSessions(userDoc, op).then(function (finalUser) {
  ...
  this.updateSession = function (key, refresh, cleanup) {
    ...
    return session.fetchToken(key).then(function (oldToken) {
      ...
      if (!newSession) return refresh ? BPromise.reject('no session for key "' + key + '"') : null;
      ...
      return userDB
        ...
        .then(function (userDoc) {
          if (!userDoc.session[key]) {
            ...
            return userDB.put(userDoc).then((result) => {
        ...
        .then(function (userDoc) {
          ...
          if (changed)
            return session.storeToken(newSession).then(function () {
              return userDoc;
            });
          else return userDoc;
        })
        .then(function (userDoc) {
          ...
          if (cleanup)
            return self.cleanupSessions(userDoc, null, refresh).then(function (finalUser) {
              user = finalUser;
              return finalUser;
            });
          else return userDoc;
        ...
        .then(function () {
          if (rolesChanged) {
            ...
            //  - update roles (couch)
            if (user.personalDBs && user.session) return dbAuth.authorizeUserSessions(user._id, user.personalDBs, [key], newSession.roles);
          }
        })
        .then(function () {
          ...
          if (changed) emitter.emit("refresh", newSession);
  ...
  this.addUserDB = function (user_id, dbName, type, designDocs, permissions) {
    ...
    return userDB
      ...
      .then(function (finalDBName) {
        ...
        if (!modified) return Promise.resolve({ ok: true, id: userDoc._id, rev: userDoc._rev });
  ...
  this.logoutUser = function (user_id, session_id) {
    ...
    return promise
      ...
      .then(function () {
        ...
        return userDB.put(user).catch(function (err) {
  ...
  function generateUsername(base, errorOnDuplicate) {
    ...
    return userDB.allDocs({ startkey: base, endkey: base + "\uffff", include_docs: false }).then(function (results) {
      ...
      if (errorOnDuplicate) return BPromise.reject("account name already exists");
      let num = 0;
      while (!finalName) {
        num++;
  ...
  function addUserDBs(newUser) {
    ...
    const processUserDBs = function (dbList, type, dbAccessName = null) {
      dbList.forEach(function (userDBName) {
        const dbConfig = dbAuth.getDBConfig(dbAccessName === null ? userDBName : dbAccessName, type === "group" ? "private" : type);
        promises.push(
          dbAuth
            .addUserDB(newUser, userDBName, dbConfig.designDocs, type, dbConfig.permissions, dbConfig.adminRoles, dbConfig.memberRoles)
            .then(function (finalDBName) {
              delete dbConfig.permissions;
              delete dbConfig.adminRoles;
              delete dbConfig.memberRoles;
              delete dbConfig.designDocs;
              dbConfig.type = type === "group" ? "private" : type;
    ...
    if (newUser.groupDb) {
      processUserDBs([newUser.groupDb], "group", defaultPrivateDBs[0]);
    } else {
