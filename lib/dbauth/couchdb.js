...
module.exports = function (couchAuthDB) {
  ...
  this.updateKey = function (key, expires, refreshed, roles) {
    ...
    return couchAuthDB.get(id).then(
      function (newKey) {
        if (!roles && !expires && !refreshed) return BPromise.resolve();
      ...
      function (error) {
        if (error.status === 404) return BPromise.resolve(); // entry for session in authDB already destroyed
  ...
  this.authorizeKeys = function (user_id, db, keys, permissions, roles) {
    ...
    return db
      ...
      .then(function (doc) {
        ...
        if (!secDoc.admins.names) {
          secDoc.admins.names = [];
        }
        let changes = false;
        keys.forEach(function (key) {
          const index = secDoc.admins.names.indexOf(key);
          if (index === -1) {
            secDoc.admins.names.push(key);
            changes = true;
          }
        });
        if (changes) {
  ...
  this.deauthorizeKeys = function (db, keys) {
    ...
    return db.get('_security').then(function (doc) {
      secDoc = doc;
      if (!secDoc.admins || !secDoc.admins.names) {
        return BPromise.resolve(false);
      }
      let changes = false;
      keys.forEach(function (key) {
        const index = secDoc.admins.names.indexOf(key);
        if (index > -1) {
          secDoc.admins.names.splice(index, 1);
          changes = true;
        }
      });
      if (changes) {
